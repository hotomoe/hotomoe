...
<SearchMarker path="/settings/preferences" :label="i18n.ts.preferences" :keywords="['general', 'preferences']" icon="ti ti-adjustments">
	<div class="_gaps_m">
		...
		<div class="_gaps_s">
			...
			<SearchMarker v-slot="slotProps" :keywords="['experimental', 'feature', 'flags', 'labs', 'laboratory']">
				<MkFolder :defaultOpen="slotProps.isParentOfTarget">
					...
					<div class="_gaps_m">
						<SearchMarker :keywords="['condensed', 'acct']">
							<MkPreferenceContainer k="enableCondensedLine">
								<MkSwitch v-model="enableCondensedLine">
									<template #label><SearchLabel>Enable condensed line</SearchLabel></template>
								</MkSwitch>
							</MkPreferenceContainer>
						</SearchMarker>
						<SearchMarker :keywords="['render', 'note']">
							<MkPreferenceContainer k="skipNoteRender">
								<MkSwitch v-model="skipNoteRender">
									<template #label><SearchLabel>Enable note render skipping</SearchLabel></template>
								</MkSwitch>
							</MkPreferenceContainer>
						</SearchMarker>
						<SearchMarker :keywords="['stack', 'router']">
							<MkPreferenceContainer k="experimental.stackingRouterView">
								<MkSwitch v-model="stackingRouterView">
									<template #label><SearchLabel>Enable stacking router view</SearchLabel></template>
								</MkSwitch>
							</MkPreferenceContainer>
						</SearchMarker>
					</div>
				...
				<MkFolder :defaultOpen="slotProps.isParentOfTarget">
					<template #icon><SearchIcon><i class="ti ti-lock"></i></SearchIcon></template>
					<template #label><SearchLabel>{{ i18n.ts.hideSensitiveInformation }}</SearchLabel></template>
					...
					<div class="_gaps_m">
						...
						<MkSwitch v-model="privateMode">
							<template #label><SearchLabel>{{ i18n.ts._hideSensitiveInformation.use }}</SearchLabel></template>
							<template #caption>{{ i18n.ts._hideSensitiveInformation.about }}</template>
						</MkSwitch>
						<MkFolder v-if="privateMode">
							<template #label><SearchLabel>{{ i18n.ts._hideSensitiveInformation.directMessages }}</SearchLabel></template>
				...
				<MkFolder v-if="$i.isModerator" :defaultOpen="slotProps.isParentOfTarget">
					<template #icon><SearchIcon><i class="ti ti-beach"></i></SearchIcon></template>
					<template #label><SearchLabel>{{ i18n.ts.vacationMode }}</SearchLabel></template>
					...
					<div class="_gaps_m">
						...
						<MkSwitch v-model="isVacation" @update:modelValue="update_isVacation()">
							<template #label>{{ i18n.ts.useVacationMode }}</template>
							<template #caption>{{ i18n.ts.vacationModeDescription }}</template>
						</MkSwitch>
					</div>
				</MkFolder>
...
const isVacation = ref<boolean>($i.isVacation !== null ? $i.isVacation : false);
...
async function update_isVacation() {
	misskeyApi('i/update', {
