...
function delEdit(): void {
	...
	if ($i) {
		...
		if ($i.policies.canUseTranslator && instance.translatorAvailable && !prefer.s.showTranslateButtonInNoteFooter) {
			menuItems.push({
		...
		if (appearNote.channel && (appearNote.channel.userId === $i.id || iAmModerator)) {
			menuItems.push({ type: 'divider' });
			menuItems.push({
				type: 'parent',
